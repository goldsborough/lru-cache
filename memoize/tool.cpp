...
static llvm::cl::OptionCategory MemoizableRewriterCategory{
    "Memoizable Rewriter"};

namespace clang {
namespace ast_matchers {
/// \brief Matches declaration that has an annotate attribute with a certain
/// argument.
///
/// Given
/// \code
...
AST_MATCHER_P(Decl, isAnnotatedWith, std::string, str) {
  for (auto const* Attr : Node.attrs()) {
    if (Attr->getKind() == attr::Annotate &&
        cast<AnnotateAttr>(Attr)->getAnnotation() == str) {
      return true;
    }
  }
...
class MemoizableHandler : public MatchFinder::MatchCallback {
 public:
  ...
  virtual void run(const MatchFinder::MatchResult& Result) override {
    ...
    if (mf) {
      auto it = std::find(
          std::begin(memoizedFunctions), std::end(memoizedFunctions), mf);
      ...
      if (it == std::end(memoizedFunctions)) {
        ...
        afterss
            << "\n\n"
            << funcdecl2prototype(
                   mf, sm, lopt, mf->getNameAsString() + std::string("_cached"))
            << " {\n"
        ...
        std::transform(mf->param_begin(),
                       mf->param_end(),
                       std::back_inserter(params),
                       [](ParmVarDecl* p) { return p->getNameAsString(); });
    ...
    if (fc) {
      ...
      rewriter.ReplaceText(fc->getLocStart(),
                           mf->getNameAsString().length(),
                           mf->getNameAsString() + std::string("_cached"));
    }
  }
...
class MainConsumer : public clang::ASTConsumer {
 public:
  MainConsumer(clang::Rewriter& rewriter) : handler{rewriter} {
...
class MainAction : public clang::ASTFrontendAction {
 public:
  ...
  virtual void EndSourceFileAction() override {
    // How can the target file to write to be specified? Ask the
    // CommonOptionsParser below?
...
int main(int argc, char const* argv[]) {
  ...
  return tool.run(
      newFrontendActionFactory<MemoizableRewriter::MainAction>().get());
}
